...
class CloudFileManagerApp:
    ...
    def _delete_s3_objects_internal(self):
        ...
        self.logger.log(f"Deleted {completed}/{total} object(s).")
        self.reset_progress()
        self.refresh_s3_tree()

    def create_s3_folder(self):
        folder_name = simpledialog.askstring("New Folder", "Enter new S3 folder name:")
        if folder_name:
            self.s3.create_folder(folder_name)
            self.logger.log(f"Created folder: {folder_name}")
            self.refresh_s3_tree()

    def view_object_url(self):
        selected_nodes = self.s3_tree.selection()
        if not selected_nodes:
            messagebox.showwarning("Select Object", "Select an S3 file.")
            return
        for node in selected_nodes:
            key = self.s3_tree.item(node, "values")[0]
    ...
    def refresh_s3_tree(self):
        ...
        self.populate_s3_root()
        self.logger.log("Refreshed S3 tree view.")
